...
class ProductoCocineroController extends Producto implements IApiUsable
{
  ...
  public function ModificarUno($request, $response, $args)
  {
    ...
    if (Producto::verificarId($id)) {
      if (Producto::traerTipo($id)["id_tipo"] == 2) {
        Producto::setEstado($id, $id_estado);
        Producto::setTiempoDeFinalizacion($id, $tiempo_finalizacion);
        $payload = json_encode(array("mensaje" => "Producto modificado con exito"));
      } else {
        $payload = json_encode(array("mensaje" => "ERROR - solo puede modificar productos de cocinero, reingrese ID"));
      }
    } else {
